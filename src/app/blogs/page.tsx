  ...
  return (
    <div className="min-h-screen bg-[#FAFAFC] dark:bg-[#0D0E15] py-12 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="relative rounded-xl bg-gradient-to-r from-[#1A1A2E] via-[#8B2E3B] to-[#FF7A00] p-8 sm:p-12 text-white shadow-lg overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl space-y-4">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-md text-xs font-bold bg-white/10 text-white border border-white/20">
              ...
              <span>SRKR CODING CLUB KNOWLEDGE HUB</span>
            </div>

            <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight leading-tight">
              Technical Blogs & Member Stories
            </h1>

            <p className="text-sm sm:text-base text-slate-200 leading-relaxed">
              In-depth technical tutorials, hackathon winning blueprints, system architecture write-ups, and career guidance written by SRKRCC developers.
            </p>
          </div>
        </div>
        ...
        {/* Featured Post Card */}
        {featuredPost && (
          <div className="relative bg-white dark:bg-[#151722] rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-md grid grid-cols-1 lg:grid-cols-12 group">
            <div className="lg:col-span-7 relative h-64 lg:h-auto overflow-hidden bg-slate-900">
              <img
                src={featuredPost.image_url || featuredPost.cover_image || 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?auto=format&fit=crop&w=1200&q=80'}
                alt={featuredPost.title}
                className="w-full h-full object-cover group-hover:scale-105 transition duration-500 opacity-90"
              />
              <div className="absolute top-4 left-4 bg-[#FF7A00] text-white px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider flex items-center space-x-1 shadow">
            ...
            <div className="lg:col-span-5 p-6 sm:p-8 flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                <div className="flex items-center space-x-3 text-xs text-slate-500 dark:text-slate-400 font-medium">
                  <span className="px-2.5 py-1 rounded bg-orange-50 dark:bg-orange-950/40 text-[#FF7A00] font-bold">
                    {featuredPost.category || 'Tech Article'}
                  </span>
                ...
                <h2 className="text-2xl font-extrabold text-[#1A1A2E] dark:text-white group-hover:text-[#FF7A00] transition leading-snug">
                  {featuredPost.title}
                </h2>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {featuredPost.tags?.map((t) => (
                    <span
                      key={t}
                      className="px-2.5 py-1 rounded text-[11px] font-mono font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300"
                    >
                      #{t}
                    </span>
                  ))}
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                ...
                <button
                  onClick={() => alert(`Reading article: "${featuredPost.title}"`)}
