...
export default function PillButton({
  ...
  return (
    ...
    >
      {/* Hover: wash the whole pill to a solid orange shade */}
      {variant === 'solid' && (
        <span className="absolute inset-0 bg-[#FF7A00] opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      )}
      ...
      {icon === 'code' && (
        <span className="relative z-10 font-mono text-xs opacity-70 group-hover:opacity-100 transition-opacity">{'</>'}</span>
      )}
    </Link>
  );
}
