    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-base font-bold text-white truncate">{member.name || 'Club Member'}</h3>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-slate-800/40 rounded-xl p-3.5 border border-slate-800">
          ...
          <div className="text-xl font-bold text-white mt-0.5">{member.total_submissions}</div>
        </div>
        <div className="bg-slate-800/40 rounded-xl p-3.5 border border-slate-800">
        ...
        <div className="space-y-2">
          {member.forms_submitted.length === 0 ? (
            ...
            member.forms_submitted.map((sub, i) => (
              <div
                key={i}
                className="p-3 rounded-xl bg-[#151722] border border-slate-800 flex items-center justify-between gap-3"
              >
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-white truncate">{sub.form_title}</p>
      ...
      <div className="space-y-2.5">
        ...
        <textarea
          ...
          className="w-full px-3.5 py-2.5 bg-[#0f0f1a] border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-orange-500/60"
        />
      ...
      <div className="pt-2 border-t border-slate-800">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-slate-700 hover:border-slate-600 text-xs font-semibold text-slate-300 hover:text-white transition"
        >
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 text-sm bg-[#151722] border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-orange-500/60"
            />
          </div>
          <select
            value={selectedFormId}
            onChange={(e) => setSelectedFormId(e.target.value)}
            className="px-3.5 py-2 bg-[#151722] border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:border-orange-500/60"
          >
        ...
        {loading ? (
          ...
          <div className="py-16 text-center text-slate-500 text-sm bg-[#151722] rounded-xl border border-slate-800">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex flex-wrap gap-1.5 max-w-[340px]">
                          {member.forms_submitted.slice(0, 3).map((f, i) => (
                            <span
                              key={i}
                              className="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-[10px] text-slate-300 truncate max-w-[150px]"
                              title={f.form_title}
                            >
                              {f.form_title}
                            </span>
